...
namespace Arana.DoubleClickForPublishers.Drivers {
   ...
   public class GooglePublisherTagFieldDriver : ContentFieldDriver<GooglePublisherTagField> {
      ...
      protected override DriverResult Display(ContentPart part, GooglePublisherTagField field, string displayType,
                                              dynamic shapeHelper) {
         return ContentShape("Fields_DoubleClickForPublishers_GPT", field.Name,
                             () => {
                                ...
                                if (!string.IsNullOrWhiteSpace(zone)) {
                                   ...
                                   if (position > 0) {
                                      zoneSection.Add(shape, position.ToString(CultureInfo.InvariantCulture));
                                   }
                                   else {
                                      zoneSection.Add(shape);
                                   }
      ...
      protected override DriverResult Editor(ContentPart part,
                                             GooglePublisherTagField field, dynamic shapeHelper) {
      ...
      protected override void Importing(ContentPart part, GooglePublisherTagField field,
                                        ImportContentContext context) {
         context.ImportAttribute(GetPrefix(field, part), "Targeting", val => field.Targeting = val);
      }

      protected override void Exporting(ContentPart part, GooglePublisherTagField field,
